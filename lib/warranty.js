...
// 型号→品牌归一化的单一事实源：invoice-reader 的 FAMILIES（27 张真实发票萃取）。
// require 无副作用（normalizeModel 是纯函数，不触网）。
...
// 型号 → 品牌映射
//
// 主映射来自 invoice-reader 的 FAMILIES（27 张真实经销商发票萃取，
// 见 docs/model-normalization.md）。**PROVISIONAL —— 待 Fanz 书面确认**：
// 发票上绝大多数型号那行直接印了品牌（FANZ-/VIOZ-），可信度高；识别不出的
// 仍返回 unknown（bot 先问品牌、不下保修结论）。Fanz 正式确认后，若某型号
// 归属与发票不符，在下面 MODEL_BRAND_MAP 手工覆盖即可（优先级高于归一化）。
//
// 客户面保修红线不受影响：照片凭证永不自动判（人工核实），且每条判定都带
// "以最新官方政策为准 / 师傅上门确认为准"免责，即使映射偶有偏差也有兜底。
// ============================================
const MODEL_BRAND_MAP = {
  // Fanz 确认后如需纠正某型号，在此覆盖，例如：
  // 'somemodel': 'vioz',
};
...
/**
 * 从客户报的型号文本推断品牌。
 * 顺序：手工覆盖(MODEL_BRAND_MAP) → 前缀 → 发票归一化(normalizeModel)。
...
function inferBrand(modelText) {
  ...
  // 2) 发票实证归一化（provisional，主力来源）
  const n = normalizeModel(modelText);
  if (n.brand === 'fanz' || n.brand === 'vioz') return n.brand;
  return 'unknown';
}
